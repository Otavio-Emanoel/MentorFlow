...
export const LoginScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Stagger dos itens do formulário
  ...
  const animateOutThenNavigate = (route: 'Register' | 'Auth' | 'Login' = 'Register') => {
      ...
      <View style={styles.contentContainer}>
        <Animated.View style={styles.contentClip}>
          ...
          >
            <Animated.Text style={[
              styles.title,
              { opacity: itemAnims[0], transform: [{ translateY: itemAnims[0].interpolate({ inputRange: [0,1], outputRange: [8,0] }) }] }
            ]}>Login</Animated.Text>
            ...
            <Animated.View style={{ width: '100%', maxWidth: 420, opacity: itemAnims[2], transform: [{ translateY: itemAnims[2].interpolate({ inputRange: [0,1], outputRange: [8,0] }) }] }}>
              ...
              />
              <Pressable style={styles.subtleLink} onPress={() => { /* TODO: forgot password */ }}>
                <Text style={styles.subtleLinkText}>Esqueceu a senha?</Text>
              </Pressable>
            </Animated.View>

            <Animated.View style={{ width: '100%', maxWidth: 420, opacity: itemAnims[3], transform: [{ translateY: itemAnims[3].interpolate({ inputRange: [0,1], outputRange: [8,0] }) }], marginTop: 4 }}>
              <Pressable style={styles.primaryButton} onPress={() => {}} android_ripple={{ color: '#ffffff33' }}>
                <Text style={styles.primaryButtonText}>Entrar</Text>
              </Pressable>
            </Animated.View>

            <Animated.View style={{ opacity: itemAnims[4], transform: [{ translateY: itemAnims[4].interpolate({ inputRange: [0,1], outputRange: [8,0] }) }], marginTop: 12 }}>
              <View style={styles.switchRow}>
                <Text style={styles.switchLabel}>Não tem conta?</Text>
                <Pressable onPress={() => animateOutThenNavigate('Register')}>
...
const styles = StyleSheet.create({
  ...
  subtleLink: { alignSelf: 'flex-end', marginTop: 2 },
  subtleLinkText: { color: theme.muted, fontSize: 12 },
